  ...
  return (
    <div className="relative flex h-full w-full flex-col justify-center overflow-hidden bg-gradient-to-br from-zinc-900 via-zinc-950 to-black p-1.5 sm:p-2 md:p-5 lg:p-6">
      ...
      <div className="relative z-10 flex h-full w-full flex-col items-center justify-start gap-1 pt-1 text-center md:flex-row md:items-center md:justify-between md:gap-5 md:text-left">
        {/* Portrait — stacked on mobile, left side on desktop */}
        <div className="relative flex h-16 w-16 flex-col items-center justify-center md:flex md:h-auto md:w-auto md:flex-[0_0_44%] md:items-center md:justify-center">
          {/* Glow pulse */}
          <motion.div
            className="absolute h-[85%] w-[85%] rounded-full bg-accent/20 blur-md md:h-[75%] md:w-[75%] md:blur-xl"
            animate={{ scale: [1, 1.05, 1], opacity: [0.3, 0.5, 0.3] }}
            transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }}
          />
          {/* Rotating ring 1 */}
          <motion.div
            className="absolute h-[80%] w-[80%] rounded-full border border-accent/25 md:h-[72%] md:w-[72%]"
            animate={{ rotate: 360 }}
          ...
          {/* Rotating ring 2 */}
          <motion.div
            className="absolute h-[65%] w-[65%] rounded-full border border-accent/15 md:h-[58%] md:w-[58%]"
            animate={{ rotate: -360 }}
          ...
          {/* Portrait image */}
          <motion.div
            className="relative z-10 h-14 w-14 overflow-hidden rounded-full ring-1.5 ring-accent/30 ring-offset-1 ring-offset-zinc-950 md:h-auto md:w-[68%]"
            animate={{ y: [0, -1.5, 0] }}
        ...
        {/* Text section — stacked below portrait on mobile, right side on desktop */}
        <div className="flex w-full flex-col items-center gap-1 md:flex-[1_1_56%] md:items-start md:gap-2">
          {/* Name — below picture on mobile, at top on desktop */}
          <motion.h2
          ...
          {/* Badge */}
          <motion.div
            className="inline-flex w-fit items-center gap-1 rounded-full bg-accent/10 px-2 py-0.5 text-[10px] font-medium text-accent ring-1 ring-inset ring-accent/20 md:px-3 md:py-1 md:text-xs"
            initial={{ opacity: 0, y: -4 }}
            animate={isActive ? { opacity: 1, y: 0 } : { opacity: 0, y: -4 }}
          ...
          {/* Headline */}
          <motion.h1
            className="font-display text-[13px] font-medium leading-tight tracking-tight md:text-xl lg:text-2xl"
          ...
          >
            <div className="text-center md:text-left">
              <p className="font-display text-[13px] font-bold text-foreground md:text-2xl lg:text-3xl">
                2+
              </p>
              <p className="text-[8px] text-muted-foreground md:text-xs">
                Years exp.
              </p>
            </div>
            <div className="text-center md:text-left">
          ...
          >
            <button
              ...
              className="group inline-flex items-center gap-0.5 rounded-full bg-accent px-2 py-0.5 text-[9px] font-medium text-zinc-950 transition-all hover:bg-accent/90 hover:shadow-[0_0_12px_rgba(74,222,128,0.4)] md:px-5 md:py-1.5 md:text-sm md:gap-1.5"
            >
              See work
            ...
            <button
              ...
              className="inline-flex items-center gap-0.5 rounded-full border border-border/60 px-2 py-0.5 text-[9px] font-medium transition-colors hover:border-accent/50 hover:bg-accent/10 hover:text-accent md:px-5 md:py-1.5 md:text-sm md:gap-1.5"
            >
              Quote
          ...
          {/* Skills row — smaller, only visible bottom on desktop, below CTAs on mobile */}
          <motion.div
            className="w-full border-t border-white/5 pt-0.5 mt-0.5 md:mt-1.5 md:pt-2"
            initial={{ opacity: 0, y: 4 }}
            animate={isActive ? { opacity: 1, y: 0 } : { opacity: 0, y: 4 }}
            transition={{ duration: 0.4, delay: 0.6 }}
          >
            <p className="mb-0.5 text-center text-[7px] font-medium uppercase tracking-[0.15em] text-muted-foreground/60 md:mb-1.5 md:text-left md:text-[10px]">
              Tools I work with
            </p>
            <div className="flex flex-wrap justify-center gap-0.5 md:justify-start md:gap-1">
              {skills.map((skill, i) => (
                <motion.span
                  key={skill.name}
                  className={`rounded-full bg-white/[0.03] px-1 py-[2px] text-[6.5px] font-medium ring-1 ring-white/5 ${skill.color} md:px-2 md:py-1 md:text-[11px]`}
                  initial={{ opacity: 0, y: 3 }}
                  animate={isActive ? { opacity: 1, y: 0 } : { opacity: 0, y: 3 }}
